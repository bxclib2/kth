...
public class SearchGUI extends JFrame {
	...
	JToggleButton efficientFeedbackExecutor = new JToggleButton("New (Efficient) search");
    ...
    feedbackBar.add( efficientFeedbackExecutor );
	p.add( feedbackBar );
	// Show the interface
	setVisible( true );
	...
	Action relevanceFeedbackSearch = new AbstractAction() { 
		public void actionPerformed( ActionEvent e ) {
			...
			query.relevanceFeedback( results, docIsRelevant, indexer, false );
	...
	Action efficientRelevanceFeedbackSearch = new AbstractAction() {
			public void actionPerformed( ActionEvent e ) {
				// Check that a ranked search has been made prior to the relevance feedback
				StringBuffer buf = new StringBuffer();
				if (( results != null ) && ( queryType == Index.RANKED_QUERY )) {
					...
					// Expand the current search query with the documents marked as relevant
					query.relevanceFeedback( results, docIsRelevant, indexer, true );
	...
	efficientFeedbackExecutor.addActionListener( efficientRelevanceFeedbackSearch );
