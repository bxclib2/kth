...
public class SPIMInvert implements Serializable {
    ...
    static private final String SPIMI_FOLDER_NAME = "spimi";
    static private final File cachedFileIndexPath = new File(Paths.get(SPIMI_FOLDER_NAME, "fileIndex").toString());

    private final File cacheFolder;
    private final File indexFolder;

    private Map<String, File> fileIndex = new HashMap<>();
    private Map<String, Integer> indexOffset = new HashMap<>();

    private SimpleCache<String, PostingsList[]> cache = new SimpleCache(10000);

    public SPIMInvert() {
        File folder = new File(SPIMI_FOLDER_NAME);
        folder.mkdir();

        cacheFolder = new File(folder, "cache");
        cacheFolder.mkdir();

        indexFolder = new File(folder, "index");
        indexFolder.mkdir();
    }

    public void save() throws Exception {
        FileOutputStream fileOut =
                new FileOutputStream(cachedFileIndexPath);
    ...
    public static SPIMInvert load() {
        ...
        try {
            FileInputStream fileIn = new FileInputStream(cachedFileIndexPath);
    ...
    public void flushBlock() {
        if (0 < postings.size()) {
            ...
            Path path = Paths.get(cacheFolder.toString(), String.valueOf(currentIndexSize) + ".spimi");
            writeBlockToDisk(path.toString());
        }
    }
    ...
    private File getFilePath() {
        ...
        return new File(indexFolder, fileName);
    }
    ...
    public PostingsList getPostings(String term) {
        ...
        int offset = indexOffset.get(term);

        if (cache.containsKey(term)) {
            PostingsList[] postingsLists = cache.get(term);
            pl = postingsLists[offset];
